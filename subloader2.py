...
class SubLoader(ctk.CTk):
    def __init__(self):
        ...
        self.title("Sub Loader v1.1 (UE4SS)")
        self.geometry("1100x750")

        # Runtime-fallback variable for tracking the path before configuration location is resolved
        self.temp_exe_path = ""
        
        # Initial search/load sequence
        ...
        self.profiles = self.config.get("profiles", {"Default": {}})
        ...
        self.log(f"Profile loaded: {self.current_profile}", "INFO")
        self.log("Welcome to Sub Loader 2", "INFO")

    def get_config_file_path(self):
        """Determines the configuration file path inside the UE4SS Mods folder structure."""
        exe_file = self.exe_path.get() if hasattr(self, 'exe_path') else self.temp_exe_path
        if not exe_file:
            return None
        game_root_folder = os.path.dirname(exe_file)
        mods_dir = os.path.join(game_root_folder, "Subnautica2", "Binaries", "Win64", "ue4ss", "Mods")
        if os.path.exists(mods_dir):
            return os.path.join(mods_dir, "subloader_config.json")
        return None
    ...
    def create_sidebar(self):
        ...
        # LAUNCH GAME BUTTON
        self.btn_launch = ctk.CTkButton(self.sidebar_frame, text="🚀 LAUNCH GAME", font=ctk.CTkFont(weight="bold", size=14), fg_color="#ff5722", hover_color="#e64a19", height=40, command=self.launch_game)
        self.btn_launch.grid(row=1, column=0, padx=20, pady=(0, 15), sticky="ew")

        # PROFILES SYSTEM DROPDOWN
        self.profile_label = ctk.CTkLabel(self.sidebar_frame, text="Active Profile:", font=ctk.CTkFont(size=11), text_color="#aaaaaa")
        self.profile_label.grid(row=2, column=0, padx=20, pady=(5, 0), sticky="w")
        
        profile_list = list(self.profiles.keys()) + ["+ Create New Profile"]
        self.profile_menu = ctk.CTkOptionMenu(self.sidebar_frame, values=profile_list, command=self.handle_profile_change)
        self.profile_menu.set(self.current_profile)
        self.profile_menu.grid(row=3, column=0, padx=20, pady=(0, 5), sticky="ew")

        # DELETE PROFILE BUTTON
        self.btn_del_profile = ctk.CTkButton(self.sidebar_frame, text="🗑 Delete Profile", font=ctk.CTkFont(size=11), fg_color="#dc3545", hover_color="#bd2130", height=24, command=self.delete_profile)
        self.btn_del_profile.grid(row=4, column=0, padx=20, pady=(0, 15), sticky="ew")

        # Navigation & Tools
        self.btn_nav_mods = ctk.CTkButton(self.sidebar_frame, text="Manage Mods", command=self.show_mod_manager)
        self.btn_nav_mods.grid(row=5, column=0, padx=20, pady=5, sticky="ew")

        self.btn_nav_install = ctk.CTkButton(self.sidebar_frame, text="Installer", command=self.show_installer)
        self.btn_nav_install.grid(row=6, column=0, padx=20, pady=5, sticky="ew")

        self.btn_add = ctk.CTkButton(self.sidebar_frame, text="Add Mod Files", command=self.select_files)
        self.btn_add.grid(row=7, column=0, padx=20, pady=5, sticky="ew")

        self.btn_web = ctk.CTkButton(self.sidebar_frame, text="Visit Nexus", command=self.open_website)
        self.btn_web.grid(row=8, column=0, padx=20, pady=5, sticky="ew")
        ...
        self.btn_auto_detect.grid(row=9, column=0, padx=20, pady=5, sticky="ew")
        ...
        self.btn_path.grid(row=10, column=0, padx=20, pady=5, sticky="ew")

        self.appearance_mode_menu = ctk.CTkOptionMenu(self.sidebar_frame, values=["Dark", "Light"], command=ctk.set_appearance_mode)
        self.appearance_mode_menu.grid(row=12, column=0, padx=20, pady=(10, 20), sticky="ew")
    ...
    def launch_game(self):
        ...
        if not exe or not os.path.exists(exe):
            messagebox.showerror("Error", "Game executable path is missing or invalid. Please select it first.")
            return
        
        def run():
            try:
                self.log("Launching Subnautica 2...", "INFO")
                subprocess.Popen([exe], cwd=os.path.dirname(exe))
            except Exception as e:
                self.log(f"Failed to launch game: {str(e)}", "ERROR")
        
        threading.Thread(target=run, daemon=True).start()

    # PROFILES SYSTEM CONTROLS
    def handle_profile_change(self, choice):
        if choice == "+ Create New Profile":
            ...
            if name and name.strip() and name.strip() not in self.profiles:
                ...
                self.log(f"Created and switched to profile: {name}", "SUCCESS")
                self.refresh_mod_list()
            else:
                self.profile_menu.set(self.current_profile)
        else:
            ...
            self.save_all_config()
            self.log(f"Switched to profile: {choice}", "INFO")
            self.apply_profile_states()
            self.refresh_mod_list()
    ...
    def delete_profile(self):
        ...
        if messagebox.askyesno("Confirm Delete", f"Are you sure you want to delete the profile '{target}'?"):
            ...
            self.save_all_config()
            self.update_profile_dropdown()
            self.log(f"Deleted profile: {target}. Switched to: {self.current_profile}", "WARNING")
            self.apply_profile_states()
            self.refresh_mod_list()

    def update_profile_dropdown(self):
        profile_list = list(self.profiles.keys()) + ["+ Create New Profile"]
        self.profile_menu.configure(values=profile_list)
        self.profile_menu.set(self.current_profile)

    def apply_profile_states(self):
        ...
        for folder_name in os.listdir(mods_dir):
            ...
            if os.path.isdir(folder_path) and folder_name.lower() not in IGNORED_MODS:
                should_be_enabled = profile_data.get(folder_name, True)
    ...
    def save_mod_state_to_profile(self, folder_name, is_enabled):
        if self.current_profile not in self.profiles:
            self.profiles[self.current_profile] = {}
        self.profiles[self.current_profile][folder_name] = is_enabled
        self.save_all_config()
    ...
    def auto_detect_exe(self, silent=False):
        ...
        if found_path:
            self.temp_exe_path = found_path
            if hasattr(self, 'exe_path'):
                self.exe_path.set(found_path)
            
            # Now that the path is detected, reload configuration from the new game mods directory location
            new_config = self.load_config()
            if new_config:
                self.profiles = new_config.get("profiles", {"Default": {}})
                self.current_profile = new_config.get("current_profile", "Default")
    ...
    def refresh_mod_list(self):
        ...
        for folder_name in os.listdir(mods_dir):
            ...
            if os.path.isdir(folder_path) and folder_name.lower() not in IGNORED_MODS:
                ...
                self.create_mod_entry(display_name, folder_name, folder_path, is_disabled)
        
        if not found_folders:
            ctk.CTkLabel(self.mods_scroll_frame, text="No custom mod folders found.").pack(pady=20)

    def create_mod_entry(self, display_name, folder_name, folder_path, is_disabled):
        ...
        def toggle():
            ...
            if switch_var.get() == "on":
                if os.path.exists(disabled_file):
                    os.rename(disabled_file, enabled_file)
                container_frame.configure(fg_color="#2d2d2d")
                lbl.configure(text_color="white")
                switch.configure(text="Enabled")
                self.save_mod_state_to_profile(folder_name, True)
            else:
                if os.path.exists(enabled_file):
                    os.rename(enabled_file, disabled_file)
                container_frame.configure(fg_color="#242424")
                lbl.configure(text_color="#777777")
                switch.configure(text="Disabled")
                self.save_mod_state_to_profile(folder_name, False)
        ...
        config_panel = ctk.CTkFrame(container_frame, fg_color="#1e1e1e")
        ...
        def load_config_content():
            if "text_area" in config_widgets:
                config_widgets["text_area"].delete("1.0", "end")
                if os.path.exists(config_path):
                    try:
                        with open(config_path, "r", encoding="utf-8") as f:
                            config_widgets["text_area"].insert("1.0", f.read())
                    except Exception as e:
                        config_widgets["text_area"].insert("1.0", f"-- Error loading file: {str(e)}")
                else:
                    config_widgets["text_area"].insert("1.0", "-- config.lua not found in scripts/. Click Save to create it.")
        ...
        def save_config_content():
            if "text_area" in config_widgets:
                ...
                try:
                    ...
                    content = config_widgets["text_area"].get("1.0", "end-1c")
        ...
        def toggle_config_view():
            if not config_is_open[0]:
                if not config_created[0]:
                    text_area = ctk.CTkTextbox(config_panel, font=("Consolas", 12), height=150, fg_color="#121212")
                    text_area.pack(fill="x", padx=10, pady=10)
                    config_widgets["text_area"] = text_area
                    ...
                    config_widgets["btn_save"] = btn_save
                    config_created[0] = True
    ...
    def load_config(self):
        """Loads config directly from the game's UE4SS Mods folder if possible, keeping the script folder perfectly clean."""
    ...
    def save_all_config(self):
        """Saves all game configurations and profile data directly into the game's UE4SS Mods folder layout."""
        config_path = self.get_config_file_path()
        if not config_path:
            return  # No target game directory is loaded or resolved yet

        payload = {
            "last_exe_path": self.exe_path.get(),
    ...
    def select_game_exe(self):
        ...
        if path:
            ...
            # Since a new directory is set, load any pre-existing configuration from that specific mods folder
            new_config = self.load_config()
            if new_config:
                self.profiles = new_config.get("profiles", {"Default": {}})
                self.current_profile = new_config.get("current_profile", "Default")
